...
public class DrawUserPair : IMediatorSubscriber, IDisposable
{
    ...
    private void DrawCommonClientMenu()
    {
        ...
        {
            if (DrawStateColoredToggleButton(FontAwesomeIcon.UserClock, "Add to Temporary Collection Whitelist", false))
            {
                var identifierToAdd = !string.IsNullOrEmpty(_pair.UserData.Alias) ? _pair.UserData.Alias : _pair.UserData.UID;
        ...
        {
            if (DrawStateColoredToggleButton(FontAwesomeIcon.UserClock, "Remove from Temporary Collection Whitelist", true))
            {
