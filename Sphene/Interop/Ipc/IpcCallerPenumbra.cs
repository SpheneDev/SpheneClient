...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (APIAvailable)
        {
            try
            {
                // Wait for cleanup with a short timeout - don't block indefinitely
                var cleanupTask = CleanupAllTemporaryCollectionsAsync(Logger);
    ...
    {
        if (!APIAvailable || _isDisposed) return Guid.Empty;
